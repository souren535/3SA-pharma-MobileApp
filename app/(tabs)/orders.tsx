...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Platform, ActivityIndicator, Modal, TextInput } from 'react-native';
...
export default function OrdersScreen() {
  ...
  const filteredOrders = (Array.isArray(allOrders) ? allOrders : []).filter(order => {
    ...
    const matchesSearch = searchQuery === '' ||
      ...
      order.shop?.shop_name?.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <View style={styles.container}>
      ...
      {!isWorking ? (
        <View className="flex-1 items-center justify-center px-10">
          ...
          <Text className="text-gray-500 text-center text-sm leading-5 mb-6">
            Please mark your attendance on the Home screen to view and manage orders.
          </Text>
          <TouchableOpacity
      ...
      <View className="bg-[#F3F6F8]">
        <View className="flex-row justify-end px-5 mt-3">
          ...
          >
            <Text className="text-white font-medium text-xs mr-1">{monthNames[selectedDate.getMonth()]}</Text>
          ...
          >
            <Text className="text-white font-medium text-xs mr-1">{selectedDate.getFullYear()}</Text>
            <Ionicons name="caret-down" size={12} color="#fff" />
          </TouchableOpacity>
        </View>
          ...
          >
            {daysInMonth.map((d, i) => {
              ...
              return (
                ...
                >
                  <Text className={`text-[11px] font-bold ${isActive ? 'text-white' : 'text-gray-400'}`}>{dayNames[d.getDay()]}</Text>
                  <Text className={`text-[22px] font-bold mt-1 ${isActive ? 'text-white' : 'text-gray-800'}`}>{d.getDate()}</Text>
                </TouchableOpacity>
              );
            })}
          </ScrollView>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ paddingTop: 8, paddingBottom: 90 }} showsVerticalScrollIndicator={false}>
        {isLoading ? (
          <ActivityIndicator size="large" color="#1A3F75" style={{ marginTop: 50 }} />
        ) : filteredOrders.length === 0 ? (
          <Text className="text-center text-gray-500 mt-10">No orders found for this date.</Text>
        ) : (
        ...
        <View className="flex-1 bg-black/50 justify-center items-center px-6">
          <View className="bg-white rounded-3xl w-full p-6 shadow-xl">
            <View className="flex-row justify-between items-center mb-4">
              <Text className="text-lg font-bold text-gray-800">Select Month</Text>
            ...
            <View className="flex-row flex-wrap justify-between">
              {monthNames.map((month, index) => (
                ...
                >
                  <Text className={`font-bold ${selectedDate.getMonth() === index ? 'text-white' : 'text-gray-700'}`}>{month}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
          ...
          <View className="bg-white rounded-3xl w-full p-6 shadow-xl">
            ...
            <View className="flex-col">
              {availableYears.map((year) => (
                ...
                >
                  <Text className={`font-bold ${selectedDate.getFullYear() === year ? 'text-white' : 'text-gray-700'}`}>{year}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
